...
circle_parameters = dict(thickness=5, # colour of the good connections
...
for index,kPSets in enumerate(allPoints): 
  ...
  for keyPoints in kPSets:
    ...
    # for all rows (y)
    for i in range(lowerIbound,upperIbound):
      # for all the columns (x)
      for j in range(lowerJbound,upperJbound):
        windowIntensity.append(img1[i][j][0])


    #find average intensity of each window
    descriptor[index].append(calc_average(windowIntensity))
